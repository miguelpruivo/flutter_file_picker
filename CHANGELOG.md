...
### Android
- When `copyToCache` is false, Android uses persistable `content://` URI permissions and avoids creating cached copies for picked files.

### iOS
- When `copyToCache` is false for document picks, iOS previously returned a platform reference instead of forcing a temporary copy. That behavior has been removed.
